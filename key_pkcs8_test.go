package ncrypto_test

//
//import (
//	"github.com/smartwalle/ncrypto"
//	"testing"
//)
//
//func TestDecodeRSAPKCS8PrivateKey(t *testing.T) {
//	var key = ""
//	if _, err := ncrypto.DecodeRSAPKCS8PrivateKey(ncrypto.FormatPKCS8PrivateKey(key)); err != nil {
//		t.Fatal(err)
//	}
//}
//
//func TestDecodeRSAPKIXPublicKey(t *testing.T) {
//	var key = ""
//	if _, err := ncrypto.DecodeRSAPKIXPublicKey(ncrypto.FormatPKIXPublicKey(key)); err != nil {
//		t.Fatal(err)
//	}
//}
//
//func TestDecodeRSAPKCS8PrivateKey2(t *testing.T) {
//	var key = `-----BEGIN PRIVATE KEY-----

//-----END PRIVATE KEY-----
//`
//	if _, err := ncrypto.DecodeRSAPKCS8PrivateKey(ncrypto.FormatPKCS8PrivateKey(key)); err != nil {
//		t.Fatal(err)
//	}
//}
//
//func TestDecodeRSAPrivateKey3(t *testing.T) {
//	var key = `-----BEGIN RSA PRIVATE KEY-----

//-----END RSA PRIVATE KEY-----
//`
//	if _, err := ncrypto.DecodeRSAPKCS8PrivateKey([]byte(key)); err == nil {
//		t.Fatal("解析 Private Key 应该失败")
//	}
//}
